...
option task = {name: "ESL_downsample_test_bucket_1d", cron: "40 23 * * *"}

from(bucket: "test_bucket-ds-1h")
    ...
    |> to(bucket: "test_bucket-ds-1d", timeColumn: "_time", tagColumns: ["enum"], fieldFn: (r) => ({"rfpower": r.rfpower}))
...
from(bucket: "test_bucket-ds-1h")
    ...
    |> to(bucket: "test_bucket-ds-1d", timeColumn: "_time", tagColumns: ["enum"], fieldFn: (r) => ({"temp": r.temp}))
...
from(bucket: "test_bucket-ds-1h")
    ...
    |> to(bucket: "test_bucket-ds-1d", timeColumn: "_time", tagColumns: ["enum"], fieldFn: (r) => ({"voltage": r.voltage}))
